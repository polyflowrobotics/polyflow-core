...
class MissionRunnerNode(PolyflowNode):
    ...
    Reads a mission — an ordered list of commands — and executes them
    sequentially, publishing Twist messages for each step. Designed to
    feed into the Differential Drive node via cmd_vel.

    Mission format (JSON string on the ``mission`` input pin):
        [
    ...
    Input pins:
        mission   (String) — JSON-encoded mission array
    ...
    def __init__(self):
        ...
        self.register_input_pin("mission", String)
    ...
    def _parse_mission(self, raw: str) -> list:
        """Parse and validate a JSON mission string. Returns list of steps."""
        try:
    ...
    def process_input(self, pin_id: str, data):
        ...
        if pin_id == "mission":
            try:
                steps = self._parse_mission(data.data)
            ...
            if self._mission_task and not self._mission_task.done():
                ...
                self.publish_to_pin("cmd_vel", self._make_twist("stop"))
                self._publish_status("Previous mission cancelled")

            self._mission = steps
            self._mission_task = asyncio.ensure_future(self._execute_mission(steps))
